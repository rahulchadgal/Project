...
public class MovieBooking implements Q10MovieBookingInterface {
...
class Cinema {
	private int cinemaId, cityId, screenCount, screenRow, screenColumn;
...
class ShowManager implements ShowSubject {
	private ArrayList<ShowObserver> observers = new ArrayList<>();
	private HashMap<Integer, Show> cache = new HashMap<>();

	Show addShow(int showId, int movieId, Cinema cinema, int screenIndex, long startTime, long endTime) {
...
class Show {
	...
	public Show(int showId, int movieId, int screenIndex, long startTime, long endTime, Cinema cinema) {
		this.showId = showId;
	...
	public String toString() {
		return "Show(" + "showId=" + showId + ", movieId=" + movieId + ", cinemaId=" + cinema.getCinemaId()
...
class TicketBookingManager {
	...
	public List<String> bookTicket(String ticketId, Show show, int ticketsCount) {
		...
		if (!seats.containsKey(show.getShowId())) {
			seats.put(show.getShowId(), new boolean[cinema.getScreenRow()][cinema.getScreenColumn()]);
			freeSeatsCount.put(show.getShowId(), cinema.getScreenRow() * cinema.getScreenColumn());
		}
		if (freeSeatsCount.get(show.getShowId()) < ticketsCount)
			return ans;
		// update seats count
		...
		outer: for (int row = 0; row < cinema.getScreenRow(); row++)
		...
		if (ans.size() == 0)
			outer2: for (int row = 0; row < cinema.getScreenRow(); row++)
				for (int column = 0; column < cinema.getScreenColumn(); column++) {
					if (ticketsCount <= 0)
						break outer2;
	...
	private ArrayList<String> lockContinuousFreeSeats(boolean bookedSeats[], int start, int seatsCount, int row) {
	...
	public boolean cancelTicket(String ticketId) {
		...
		freeSeatsCount.put(booking.getShowId(), freeSeatsCount.get(booking.getShowId()) + booking.getSeats().size());
		return true;
	}

	public int getFreeSeatsCount(Show show) {
		Cinema cinema = show.getCinema();
		return freeSeatsCount.getOrDefault(show.getShowId(), cinema.getScreenRow() * cinema.getScreenColumn());
	}
}
...
class ShowLister implements ShowObserver {
	...
	public List<Integer> listShows(int movieId, int cinemaId) {
		...
		if (set != null) {
			set.sort((a, b) -> {
				return a.getStartTime() != b.getStartTime() ? b.getStartTime().compareTo(a.getStartTime())
...
/*
 * class ShowLister implements ShowObserver { private HashMap<String,
 * TreeSet<Show>> cache = new HashMap<>();
 * 
 * // returns all showId's of all shows displaying the movie in // given cinema.
 * showId's are ordered in descending order // of startTime and then showId
 * public List<Integer> listShows(int movieId, int cinemaId) {
 * ArrayList<Integer> list = new ArrayList<>(); TreeSet<Show> set = cache.get(""
 * + movieId + "-" + cinemaId); if (set != null) for (Show show : set)
 * list.add(show.getShowId()); System.out.println("movieId " + movieId +
 * ", cinemaId " + cinemaId + ", list of shows " + list); return list; }
 * 
 * public void update(Show show) { String key = "" + show.getMovieId() + "-" +
 * show.getCinema().getCinemaId(); System.out.println("updating shows list: " +
 * show); cache.putIfAbsent(key, new TreeSet<Show>((a, b) -> { return
 * a.getStartTime() != b.getStartTime() ?
 * b.getStartTime().compareTo(a.getStartTime()) : a.getShowId() - b.getShowId();
 * })); cache.get(key).add(show); System.out.println(cache.get(key));
 * 
 * } }
 */
