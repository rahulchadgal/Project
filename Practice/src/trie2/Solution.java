...
public class Solution implements Q11ElevatorSystemInterface {
	...
	/**
	 * maximum count of people inside lift liftCapacity user press the outside UP or
	 * DOWN button outside lift, returns lift index or -1
	 */
	public int requestLift(int floor, char direction) {
		...
		for (int i = 0; i < lifts.length; i++) {
			...
			if (time < 0 || lift.countPeople(floor, direction) >= liftsCapacity)
				continue;
	...
	public String getLiftState(int liftIndex) {
		if (liftIndex < 0 || liftIndex >= lifts.length)
			return "";
		Lift lift = lifts[liftIndex];
		return "" + lift.getCurrentFloor() + "-" + lift.getMoveDirection() + "-" + lift.getCurrentPeopleCount();
	}

	/**
	 * This method is called every second so that lift states can be appropriately
	 * updated. we use this time rather than java.util.Date().time
	 */
	public void tick() {
...
class Lift {
	...
	/**
	 * people who are already inside lift and have chosen on which floor they will
	 * get off
	 */
	HashMap<Integer, Integer> outgoingRequestsCount;
	private LiftState movingUpState, movingDownState, idleState, movingUpToPickFirst, movingDownToPickFirst, state;
		...
		if (state.getDirection() == 'I') {
			if (floor == currentFloor)
				...
				if (floor > currentFloor)
					...
					state = direction == 'D' ? movingDownState : movingDownToPickFirst;
			}
		}
	...
	void addOutGoingRequest(int floor, char direction) {
		outgoingRequestsCount.put(floor, 1 + outgoingRequestsCount.getOrDefault(floor, 0));
	}

	/**
	 * count number of people who will be on given floor in given direction, returns
	 * -1 if direction is invalid for lift
	 */
	...
	void tick() {
		state.tick();
		// if there are no people inside lift and no incoming requests then change lift
		// state to idle
...
class MovingUpState extends LiftState {
	...
	public int getTimeToReachFloor(int floor, char direction) {
		...
		if (direction != getDirection() || floor < lift.getCurrentFloor())
			return -1;
		return floor - lift.getCurrentFloor();
	}

	// people who will enter on given floor+people who are already in and will get
	// off after floor
	public int countPeople(int floor, char direction) {
		if (direction != 'U')
			return 0;
		...
		for (int floorItr : lift.outgoingRequestsCount.keySet())
			if (floorItr > floor)
				peopleCount += lift.outgoingRequestsCount.getOrDefault(floorItr, 0);
		return peopleCount;
	}
	...
	public void tick() {
		...
		if (lift.incomingRequestsCount.isEmpty() && lift.outgoingRequestsCount.isEmpty())
			return;
...
class MovingDownState extends LiftState {
	...
	public void tick() {
		// remove old floor
		lift.incomingRequestsCount.remove(lift.getCurrentFloor());
		// check if idle state has been achieved, if there were requests on previous
		// floor but no one entered
		if (lift.incomingRequestsCount.isEmpty() && lift.outgoingRequestsCount.isEmpty())
			return;
		lift.setCurrentFloor(lift.getCurrentFloor() - 1);
		lift.outgoingRequestsCount.remove(lift.getCurrentFloor());
...
class MovingUpToPickFirstState extends LiftState {
	...
	// topmost floor till which will go up to, it won't serve requests above this
	// floor
	int nextStop() {
		int nextStop = -1;
